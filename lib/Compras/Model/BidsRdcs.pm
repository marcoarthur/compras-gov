package Compras::Model::BidsRdcs;
use Mojo::Base 'Compras::Model';
use utf8;

has doc_url => sub {
    return 'http://compras.dados.gov.br/docs/licitacoes/v1/rdcs.html';
};

has from_module => sub { 'licitacoes' };
has model_name  => sub { 'rdcs' };
...
has attributes => sub {
    return {
        ...
        forma_de_realizacao_licitacao => 'Presencial ou Eletrônico.',
...
has search_parameters => sub {
    return {
        ...
        data_publicacao            => [ 'Texto', 'Não', 'Data da publicação da licitação.' ],
        ...
        forma_de_realizacao        =>
          [ 'Texto', 'Não', 'Indica se é presencial ou eletrônico (Pregão ou RDC)' ],
